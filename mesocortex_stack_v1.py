#!/usr/bin/env python3
"""
Mesocortex Stack v1 — Unifying Mesh + Cortex

Layers in this file:

- Message, BaseModule, NodeRuntime     → root event substrate
- LoggerModule, EchoModule             → basic utilities
- NetworkMeshModule                    → TCP-based mesh between nodes
- Cortex7Core, Cortex7Module           → local synthetic cortical microcircuits
- MesocortexModule                     → global orchestrator that treats remote cortex7
                                         modules as columns in a brain-like network

Demo:
  - Two nodes: nodeA (controller) and nodeB (remote cortex)
  - nodeA has: logger, echo, mesh, cortex7_A, mesocortex
  - nodeB has: logger, echo, mesh, cortex7_B
  - Mesocortex on nodeA:
      * Stimulates region mapped to nodeB.cortex7
      * Advances all columns
      * Requests dumps
"""
...
class NodeRuntime:
    ...
    - Network, cortex, mesocortex are all just modules.
    """
...
class Cortex7Module(BaseModule):
    ...
    Emits:
    - 'cortex_spike' to logger
    - 'cortex_state' to logger on dump
    """
...
class MesocortexModule(BaseModule):
    """
    Treats cortex7 modules (local or remote) as "columns" in regions.
        ...
        "B_primary": [
            {
                "id": "col_B1",
                "node": "nodeB",
                "module": "cortex7",  # module name on that node
            },
            {
                "id": "col_A1",
                "node": "nodeA",
                "module": "cortex7",  # local cortex
            },
        ],
    }
    ...
    - kind='stimulate_region'
      ...
      Sends injects to all columns in that region (remote via mesh, local directly).

    - kind='advance_all'
        payload = { 'ticks': int }

      Sends advance to all columns.

    - kind='dump_all'
        payload = {}

      Instructs all columns to dump state.

    For remote targets, it talks to the local 'mesh' module using 'send_remote'.
    """

    def __init__(self, name: str, topo: Dict[str, List[Dict[str, Any]]]):
        super().__init__(name)
        self.regions = topo  # region_name -> list of column dicts
    ...
    async def on_message(self, msg: Message):
        if msg.kind == "stimulate_region":
            ...
            for col in cols:
                for p in pattern:
                    # fan out same pattern to every column for now
                    payload = {
...
async def run_demo():
    ...
    nodeB:
      - logger
      - echo
      - cortex7 (remote column)
      - mesh (listening 9302)

    Mesocortex on nodeA:
      - stimulates region "B_primary" that spans nodeA.cortex7 and nodeB.cortex7
      - advances all columns
      - dumps all states
    """

    peersA = {"nodeB": ("127.0.0.1", 9302)}
    peersB = {"nodeA": ("127.0.0.1", 9301)}
    ...
    nodeA.register(NetworkMeshModule("mesh", "127.0.0.1", 9301, peersA))

    # Mesocortex topology: region spans a local and a remote column
    topo = {
        "B_primary": [
    ...
    # 1) Stimulate region B_primary from mesocortex
    print("\n=== Mesocortex: stimulate region B_primary (local + remote columns) ===")
    for i in range(6):
        await nodeA.emit(
            src="testerA",
            dest="mesocortex",
    ...
    # 2) Advance all columns a bit more
    print("\n=== Mesocortex: advance_all ===")
    await nodeA.emit(
        src="testerA",
        dest="mesocortex",
